...
namespace glz
{
   namespace detail
   {
      ...
      void find_newline(auto&& it, auto&& end)
      {
         while (++it != end && *it != '\n')
            ;
      }

      void goto_delim(auto&& it, auto&& end, char delim)
      {
               ...
               while (it != end) {
                  ...
                  while (*it != '\n' && it != end) {
                     ...
                     if (brace_pos != std::string_view::npos) {
                        ...
                        int i;
                        auto result = std::from_chars(index.data(), index.data() + index.size(), i);
                        ctx.csv_index = i;
   ...
   inline void read_file_csv(T& value, const sv file_name)
   {
      context ctx{};
      ...
      read<opts{.format = csv, .row_wise = RowWise}>(value, buffer, ctx);
   }
}
